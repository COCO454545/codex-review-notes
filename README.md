...
```text
D:\Workspace\skills\workspace-project-creator\SKILL.md
D:\Workspace\skills\research-framework\SKILL.md
```

GitHub 仓库中的 `claude/skills/` 是用于跨设备同步的副本；本地工作时，以 `D:\Workspace\skills\` 作为唯一统一入口，不要把正式 Skills 散落在某个项目目录下面，也不要用单个 `.md` 文件平铺冒充 skill。
